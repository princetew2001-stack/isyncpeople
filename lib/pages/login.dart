...
class _LoginPageState extends State<LoginPage> with TickerProviderStateMixin {
    ...
    try {
      ...
      if (isLoggedIn && loginTimeStr != null) {
        ...
        if (daysDifference < 30) {
          // Auto login — but first ensure stale punch data from a previous day
          // is cleared so the clocking screen starts fresh.
          final todayStr = DateFormat('yyyy-MM-dd').format(currentTime);
          final lastResetDate = prefs.getString('last_reset_date') ?? '';
          if (lastResetDate != todayStr) {
            debugPrint('🗓️ Auto-login: new day detected ($lastResetDate → $todayStr), clearing stale punch data');
  ...
  Future<void> _saveLoginData([Map<String, dynamic>? loginResponseData]) async {
    try {
      ...
      // Mark today's date as reset date so clocking page starts fresh for the day.
      // This ensures yesterday's clocking data (if still cached) gets cleared.
      final todayStr = DateFormat('yyyy-MM-dd').format(DateTime.now());
      await prefs.setString('last_reset_date', todayStr);
      debugPrint('💾 last_reset_date set to $todayStr on login');
      ...
      if (loginResponseData != null) {
        ...
        if (loginResponseData.containsKey('punch_in_data')) {
          ...
          if (punchInData != null && punchInData is Map && punchInData.isNotEmpty) {
            // Validate that punch_in_data is from today before saving
            bool isToday = false;
            try {
              final punchTimeStr = punchInData['punch_time']?.toString();
              if (punchTimeStr != null) {
                final punchDt = DateTime.parse(punchTimeStr);
                final now = DateTime.now();
                isToday = punchDt.year == now.year &&
                    punchDt.month == now.month &&
                    punchDt.day == now.day;
              }
            } catch (_) {}
            ...
            if (isToday) {
              ...
              debugPrint('💾 User already punched in today - saved punch_in_data');
            } else {
              // Punch data is from a previous day — clear it so stale data
              // doesn't show "Day Complete" on a fresh day
              await prefs.remove('punch_in_data');
              await prefs.setBool('is_clocked_in', false);
              await prefs.remove('check_in_time');
              await prefs.remove('check_in_location');
              debugPrint('🗑️ punch_in_data is from a previous day, cleared');
            }
          } else {
        ...
        if (loginResponseData.containsKey('punch_out_data')) {
          ...
          if (punchOutData != null && punchOutData is Map && punchOutData.isNotEmpty) {
            // Validate that punch_out_data is from today before saving
            bool isToday = false;
            try {
            ...
            if (isToday) {
              ...
              debugPrint('💾 User already punched out today - saved punch_out_data');
            } else {
              // Punch out data from a previous day — clear it
              await prefs.remove('punch_out_data');
              await prefs.setBool('is_clocked_out', false);
              await prefs.remove('check_out_time');
              await prefs.remove('check_out_location');
              debugPrint('🗑️ punch_out_data is from a previous day, cleared');
            }
          } else {
