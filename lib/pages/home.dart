...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _onRefresh() async {
    ...
    try {
      // Fire location in background (non-critical)
      _getCurrentLocation();
  ...
  bool _hasLeaveApprovalPermission = false;
  bool _hasApplicationApprovalPermission = false;

  // Quick access scroll
  final ScrollController _quickAccessScrollController = ScrollController();
  // Quick access permission flags for all ESS screens
  bool _hasPermDailyAttendance = false;
  bool _hasPermMissPunch = false;
  bool _hasPermLeaveApplication = false;
  bool _hasPermOnDuty = false;
  bool _hasPermPayslip = false;
  bool _hasPermTeamAttendance = false;
  bool _hasPermWFH = false;
  bool _hasPermCompOff = false;
  ...
  Future<void> _loadPermissions() async {
    try {
      ...
      _hasLeaveApprovalPermission = false;
      _hasApplicationApprovalPermission = false;
      _hasPermDailyAttendance = false;
      _hasPermMissPunch = false;
      _hasPermLeaveApplication = false;
      _hasPermOnDuty = false;
      _hasPermPayslip = false;
      _hasPermTeamAttendance = false;
      _hasPermWFH = false;
      _hasPermCompOff = false;
      ...
      _hasLeaveApprovalPermission = await _permissionService.canRead('Leave Approval');
      _hasApplicationApprovalPermission = await _permissionService.canRead('Application Approval');
      _hasPermDailyAttendance = await _permissionService.canRead('Daily Attendance');
      _hasPermMissPunch = await _permissionService.canRead('Miss Punch');
      _hasPermLeaveApplication = await _permissionService.canRead('Leave Application');
      _hasPermOnDuty = await _permissionService.canRead('On Duty');
      _hasPermPayslip = await _permissionService.canRead('Payslip');
      _hasPermTeamAttendance = await _permissionService.canRead('Team Attendance');
      _hasPermWFH = await _permissionService.canRead('WFH');
      _hasPermCompOff = await _permissionService.canRead('Comp-Off');
        ...
        child: Stack(
          children: [
            SingleChildScrollView(
              ...
              color: isDark ? const Color(0xFF1A1A2E) : const Color(0xFFE8ECF0),
            ...
            Padding(
              ...
              child: GridView.count(
                ...
                children: [
                  ...
                  _buildAttendanceCardTappable('Holiday', holidays, const Color(0xFF00897B), () { // Teal
                  ...
                  _buildAttendanceCardTappable('Week Off', weekOffs, const Color(0xFF5C6BC0), () {
            ...
            // Quick Access Buttons (horizontally scrollable, permission-based)
  ...
  Widget _buildQuickAccessSection() {
    // Build the list of permitted quick-access items
    final List<Map<String, dynamic>> items = [];

    if (_hasPermDailyAttendance) {
      items.add({
        'title': 'Daily\nAttendance',
    ...
    if (_hasLeaveApprovalPermission) {
      items.add({
        'title': 'Leave\nApproval',
        'icon': Icons.approval,
        'color': Colors.orange,
    ...
    if (_hasApplicationApprovalPermission) {
      items.add({
        'title': 'Application\nApproval',
        'icon': Icons.schedule,
        'color': Colors.red,
    ...
    // Employee Screen always shown (no specific ESS permission needed)
    items.add({
      'title': 'Employee\nScreen',
      'icon': Icons.people,
      'color': Colors.blue,
