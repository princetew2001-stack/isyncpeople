...
Future<void> _checkDailyReset() async {
  try {
    ...
    if (lastResetDateStr != currentDateStr) {
      // It's a new day, clear yesterday's completed check-out info
      ...
      // Only reset active clock-in data if user is NOT currently clocked in (preserves night shifts)
      final isClockedIn = prefs.getBool('is_currently_clocked_in') ?? false;
      if (!isClockedIn) {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        fontFamily: 'Roboto',
        useMaterial3: true,
        scaffoldBackgroundColor: const Color(0xFFF0F2F5), // soft cool-grey — easy on eyes
